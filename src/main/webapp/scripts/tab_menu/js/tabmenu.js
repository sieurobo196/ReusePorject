	...
	*/	
	...
	function showHide()
	{
		if(activeMenuItem){
			...
			document.getElementById('tab_submenu_'+theId).style.display='none';
		...
		document.getElementById('tab_submenu_'+theId).style.display='block';
	...
	function initMenu()
	{
		var mainMenuObj = document.getElementById('tab_mainMenu');
		var menuItems = mainMenuObj.getElementsByTagName('A');
		if(document.all){
			mainMenuObj.style.visibility = 'hidden';
			document.getElementById('tab_submenu').style.visibility='hidden';
		}		
		if(rememberActiveTabByCookie){
		...
		while(document.getElementById('tab_submenu_' + subCounter)){

			var subItem = document.getElementById('tab_submenu_' + subCounter);
			
			/*if(leftAlignSubItems){
				// No action
			}else{							
				var leftPos = mainMenuLinks[subCounter-1].offsetLeft;
				document.getElementById('tab_submenu_'+subCounter).style.paddingLeft =  leftPos + 'px';
		...
		if(document.all){
			...
			document.getElementById('tab_submenu').style.visibility='visible';
		}		
		document.getElementById('tab_submenu').style.display='block';
	}
