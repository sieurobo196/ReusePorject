...
function createOwnerUser(type) {
	...
	if (type == 2) { // select POS
		...
		sel.append('<option value="ROLE_RSM">Quản Trị Miền</option>');
	}
	sel.select2();
}
...
function validateLatLong() {
    var isOK = true;
    var isError = false;
    if (checkEmptys('#lat', 'Vui lòng nhập vị trí.', '#pos_latlng_error') ||
        checkEmptys('#lng', 'Vui lòng nhập vị trí.', '#pos_latlng_error')) {
        isOK &= false;
        isError = true;
    } else {
        isOK &= true;
        $('#pos_latlng_error').html('');
        isError = false;
    }
    
    if (isError) return isOK;

    var lat = $("#lat").val();
    var lng = $("#lng").val();
    if (isNumber(lat) == false || isNumber(lng) == false) {
        isOK &= false;
        isError = true;
        $('#pos_latlng_error').html('Vui lòng nhập định dạng kiểu số.');
    } else {
        isOK &= true;
        isError = false;
        $('#pos_latlng_error').html('');
    ...
    if(parseFloat(lat) <= 0 || parseFloat(lng) <= 0){
    	 isOK &= false;
        isError = true;
        $('#pos_latlng_error').html('Vui lòng nhập định kinh độ, vĩ độ lớn hơn 0');
    }else {
        isOK &= true;
        isError = false;
        $('#pos_latlng_error').html('');
    ...
    if (checkDecimal(lat) == false || checkDecimal(lng) == false) {
        isOK &= false;
        isError = true;
        $('#pos_latlng_error').html('Nhập vị trí quá giới hạn cho phép.');
    } else {
        isOK &= true;
        isError = false;
        $('#pos_latlng_error').html('');
...
function openAddForm(channelId) {
    $('#channel_full_code_parent').val(channelSelectedCode);
	clearForm();
    var type = 1;//$("#s_type").val();
    var level = 1;//parseFloat($('#capDaiLy').val());
	if (level == 1) {
		...
		if (parentId.charAt(0) == '2') { // select POS
			...
			sel.append('<option value="ROLE_ASM">Quản lý khu vực</option>');
		...
		if (showSupplier) {
			...
			$('#u_nhanVienBanhTuoi').parent().parent().show()
		} else {
			$('#u_nppId').parent().parent().hide()
			$('#u_nhanVienBanhTuoi').parent().parent().hide()
		}
		sel.select2();
	}
}
function openSearchForm(channelId) {
    openAddForm(channelId);
}
